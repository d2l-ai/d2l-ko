...
1. 이 절에서 예를 들었던 제어 흐름(control flow)의 그래디언트(gradient)를 찾고자 할 때, 변수 `a` 가 랜덤 벡터 또는 행렬에 따라서 바뀌고 있습니다. 이 때, `c` 계산 결과는 더 이상 스칼라 형태가 아닙니다. 결과에 무슨 일이 일어났나요? 이것을 어떻게 분석할 수 있나요?
