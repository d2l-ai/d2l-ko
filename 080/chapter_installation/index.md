...
실습 환경을 구성하는 가장 간단한 방법은 [Miniconda](https://conda.io/en/latest/miniconda.html)를 설치해서 사용하는 것이고, Python 3.x 버전을 요구합니다. 만약 이미 conda가 설치가 되어 있다면, 아래 단계들은 건너뛰세요. 웹 사이트에서 여러분의 시스템에 맞는 Miniconda sh 파일을 다운로드한 후, 명령창에서  `sh <FILENAME> -b` 를 입력해서 설치를 수행합니다. macOS 사용자는 아래 명령을 수행합니다.
...
CPU 및 GPU를 지원하는 TensorFlow를 다음과 같이 설치할 수 있습니다.
```bash
