...
GMF와 MLP의 결과를 융합하기 위해 NeuMF는 두 개의 서브네트워크의 두 번째 마지막 계층을 연결하여 추가 계층으로 전달할 수 있는 특징 벡터를 만듭니다.그 후 출력은 행렬 $\mathbf{h}$와 시그모이드 활성화 함수로 투영됩니다.예측 계층은 다음과 같이 공식화됩니다. $$\hat{y}_{ui} =\sigma (\mathbf {h}^\top [\mathbf {x},\phi^L (z^{(L-1)})]).$$ 
